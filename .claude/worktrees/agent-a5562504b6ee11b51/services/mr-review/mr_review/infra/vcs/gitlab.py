...
def _parse_diff_text(diff_text: str) -> list[DiffHunk]:
    ...
    for raw_line in diff_text.splitlines():
        ...
        if raw_line.startswith("+") and not raw_line.startswith("+++"):
            current_hunk.lines.append(DiffLine(type="added", new_line=new_line, content=raw_line[1:]))
            new_line += 1
        elif raw_line.startswith("-") and not raw_line.startswith("---"):
            current_hunk.lines.append(DiffLine(type="removed", old_line=old_line, content=raw_line[1:]))
            old_line += 1
        else:
            content = raw_line[1:] if raw_line.startswith(" ") else raw_line
            current_hunk.lines.append(DiffLine(type="context", old_line=old_line, new_line=new_line, content=content))
            old_line += 1
            new_line += 1
    ...
    async def list_mrs(self, repo_path: str, state: str = "opened") -> list[MR]:
        ...
        while True:
            ...
            if not data:
                break
            for item in data:
                changes_count = item.get("changes_count") or 0
                mrs.append(
                    MR(
                        iid=int(item["iid"]),
