...
@router.get("/{review_id}/prompt", response_class=Response)
async def get_review_prompt(
    review_id: UUID,
    use_case: FromDishka[GetReviewPromptUseCase],
    exclude_diff: bool = Query(default=False),
) -> Response:
    try:
        prompt = await use_case.execute(review_id, exclude_diff=exclude_diff)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
...
async def import_response(
    ...
    try:
        result = await use_case.execute(review_id=review_id, raw=body.raw)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
